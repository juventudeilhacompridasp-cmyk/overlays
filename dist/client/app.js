...
function renderSportScorebug() {
  ...
  const animation = ['assemble', 'slide', 'zoom', 'flip'].includes(appearance.scoreboardAnimation) ? appearance.scoreboardAnimation : 'assemble';
  const style = ['classic', 'glass', 'minimal', 'contrast'].includes(appearance.scoreboardStyle) ? appearance.scoreboardStyle : 'classic';
...
const OVERLAY_STYLE_OPTIONS = {
  scoreboardStyle: [['classic','Clássico TV'],['minimal','Minimalista'],['glass','Vidro'],['contrast','Alto impacto']],
...
function renderAppearanceTab() {
  const appearance = state.appearance || defaultAppearance();
  const scoreboardStyles = [['classic','Clássico','Blocos sólidos'],['glass','Vidro','Transparência'],['minimal','Minimal','Sem excesso'],['contrast','Contraste','Contorno forte']];
...
function handleAction(action, target) {
  ...
  if (action === 'scoreboard-style') {
    const style = ['classic', 'glass', 'minimal', 'contrast'].includes(target.dataset.value) ? target.dataset.value : 'classic';
...
app.addEventListener('input', event => {
  ...
  if (target.matches('[data-appearance]')) {
    ...
    commit(draft => {
      ...
      else if (key === 'scoreboardAnimation') draft.appearance[key] = ['assemble','slide','zoom','flip'].includes(target.value) ? target.value : 'assemble';
